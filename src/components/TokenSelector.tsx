  ...
  return (
    <div className="w-full">
        ...
        <div className="flex-1 relative">
          {selectedToken && !isManualInput ? (
            <button
              ...
              className="w-full bg-transparent p-2 text-sm outline-none border border-slate-600 rounded-md text-white hover:border-purple-600 transition-colors flex items-center gap-2"
            >
              <img
                src={selectedToken.image || "/vite.svg"}
            ...
            <input
              ...
              className="bg-transparent p-2 text-sm w-full outline-none border border-slate-600 rounded-md text-white focus:border-purple-600"
            />
          )}
        </div>
        <button
          type="button"
          onClick={() => setIsModalOpen(true)}
          className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-md border border-slate-600 transition-colors text-sm whitespace-nowrap"
        >
      ...
      {isModalOpen && (
        ...
        >
          <div
            className="bg-slate-800 rounded-lg max-w-2xl w-full max-h-[80vh] flex flex-col shadow-xl"
          ...
          >
            ...
            <div className="flex justify-between items-center p-6 border-b border-slate-700">
              ...
              <button
                onClick={() => setIsModalOpen(false)}
                className="text-white hover:text-purple-400 text-2xl leading-none w-8 h-8 flex items-center justify-center transition-colors"
              >
                ×
              </button>
            </div>
            ...
            <div className="p-4 border-b border-slate-700">
              <div className="relative">
                <input
                  ...
                  className="w-full bg-slate-900 border border-slate-600 rounded-md px-4 py-2 pl-10 text-white placeholder-slate-400 focus:outline-none focus:border-purple-600"
                />
                <svg
                ...
                <div className="space-y-2">
                  {filteredTokens.map((token) => (
                    <button
                      ...
                      className="w-full flex items-center gap-3 p-3 rounded-md hover:bg-slate-700 transition-colors text-left"
                    >
                      <img
                        src={token.image || "/vite.svg"}
                        alt={token.symbol}
            ...
            <div className="p-4 border-t border-slate-700">
              <button
                ...
                className="w-full py-2 px-4 bg-slate-700 hover:bg-slate-600 text-white rounded-md transition-colors text-sm"
              >
